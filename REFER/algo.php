...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"), "SELECT * FROM course ");
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $temp->subjectteacher = $row['alloted01'];
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"),
            "SELECT * FROM staff WHERE s_id='$temp->subjectteacher'");
...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"), "SELECT * FROM staff ");
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = ($sem + 1) / 2;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"),
        "SELECT name FROM class WHERE year='$year'");
    $row = mysqli_fetch_assoc($classroom_query);
    $classroom = $row['section'];
    for ($j = 0; $j < 30; $j++) {
        $subject_teacher;
        for ($z = 0; $z < $count; $z++) {
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $temp->semester = $row['semester'];
    $temp->subjectteacher = $row['alloted01'];
    $temp->subjectteacher2 = $row['alloted02'];
    $temp->subjectteacher3 = $row['alloted03'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"),
        "SELECT * FROM staff WHERE s_id='$temp->subjectteacher'");
    $row = mysqli_fetch_assoc($teacheralias_query);
    $temp->alias = $row['alias'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"),
        "SELECT * FROM staff WHERE s_id='$temp->subjectteacher2'");
    $row = mysqli_fetch_assoc($teacheralias_query);
    $temp->alias2 = $row['alias'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"),
        "SELECT * FROM staff WHERE s_id='$temp->subjectteacher3'");
    $row = mysqli_fetch_assoc($teacheralias_query);
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "dbtest"), $query);
    }
